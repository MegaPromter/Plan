...
function exitSandbox() {
    ...
    document.querySelectorAll('tr[data-sandbox-item]').forEach(function(el) { el.remove(); });

    // Убираем sandbox-подсветку со строк
    document.querySelectorAll('.sandbox-create,.sandbox-update,.sandbox-delete').forEach(function(el) {
...
function openChangesetList() {
    ...
    fetchJson(url).then(function(data) {
        ...
        var statusColors = {draft:'draft', review:'review', approved:'approved', rejected:'rejected'};
        var html = '';
        items.forEach(function(cs) {
            html += '<div class="cs-list-item" onclick="activateChangeset(' + cs.id + ')">' +
                '<span class="cs-title">' + esc(cs.title) + '</span>' +
                '<span class="sandbox-badge ' + statusColors[cs.status] + '">' + esc(cs.status_display) + '</span>' +
                '<span class="cs-count" title="Элементов">' + cs.items_count + '</span>' +
                '<span class="cs-meta">' + esc(cs.author_name) + '<br>' + _formatDate(cs.created_at) + '</span>' +
                '</div>';
        });
        body.innerHTML = html;
    }).catch(function(err) {
        body.innerHTML = '<div class="alert alert-error">Ошибка загрузки: ' + esc(err.message || String(err)) + '</div>';
    });
}
...
function createChangeset() {
    ...
    var projectId = currentPPProjectId || _getPPProjectId();
    if (!projectId) { showToast('Не определён проект ПП', 'error'); return; }
...
function activateChangeset(id) {
    ...
    fetchJson('/api/changesets/' + id + '/').then(function(cs) {
...
function _applySandboxOverlay(items) {
    ...
    document.querySelectorAll('.sandbox-action-badge').forEach(function(el) { el.remove(); });
    document.querySelectorAll('.sandbox-cell-changed').forEach(function(el) {
        el.classList.remove('sandbox-cell-changed');
    });
    ...
    items.forEach(function(item) {
        ...
        if (item.action === 'update') {
            ...
            if (item.field_changes) {
                ...
                var fieldNames = changedFields.map(function(f) { return _fieldLabel(f); });
    ...
    var createItems = items.filter(function(item) { return item.action === 'create'; });
...
function _renderSandboxCreateRows(createItems) {
    ...
    tbody.querySelectorAll('tr[data-sandbox-item]').forEach(function(el) { el.remove(); });
    ...
    createItems.forEach(function(item, idx) {
        ...
        html += '<td style="text-align:center;white-space:nowrap;">' +
            '<button class="btn-delete sandbox-remove-item" data-item-id="' + item.id + '" title="Удалить из песочницы">' +
...
function approveChangeset() {
    if (!currentChangesetId) return;
    confirmDialog('Утвердить и применить все изменения набора «' + (currentChangeset?.title || '') + '»?')
        .then(function(ok) {
            if (!ok) return;
...
function rejectChangeset() {
    ...
    if (!comment) { showToast('Укажите причину отклонения', 'warning'); return; }
...
function _showConflicts(conflicts) {
    var html = '<div class="diff-summary" style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);">' +
        '<strong>⚠️ Конфликты (' + conflicts.length + '):</strong></div>';
    conflicts.forEach(function(c) {
        html += '<div class="diff-section"><div class="diff-section-header" style="background:rgba(239,68,68,0.06);">' +
            'Строка #' + (c.target_row_id || c.item_id) + ' — поле «' + esc(c.field || c.error || '') + '»</div>' +
            '<div class="diff-field-row">' +
            '<span class="diff-field-name">Ожидалось</span><span class="diff-field-old">' + esc(String(c.expected || '')) + '</span>' +
            '</div><div class="diff-field-row">' +
            '<span class="diff-field-name">Сейчас</span><span class="diff-field-new">' + esc(String(c.actual || '')) + '</span>' +
            '</div></div>';
    });
...
function showChangesetDiff() {
    ...
    fetchJson('/api/changesets/' + currentChangesetId + '/diff/').then(function(data) {
        ...
        var html = '<div class="diff-summary">' +
            '<div class="diff-summary-item"><span class="diff-summary-count" style="color:var(--success);">' + (summary.creates || 0) + '</span> созданий</div>' +
            '<div class="diff-summary-item"><span class="diff-summary-count" style="color:var(--warning);">' + (summary.updates || 0) + '</span> изменений</div>' +
            '<div class="diff-summary-item"><span class="diff-summary-count" style="color:var(--danger);">' + (summary.deletes || 0) + '</span> удалений</div>' +
            '</div>';

        diff.forEach(function(item) {
            var actionClass = item.action === 'create' ? 'create' : item.action === 'delete' ? 'delete' : 'update';
            var icon = item.action === 'create' ? '➕' : item.action === 'delete' ? '🗑️' : '✏️';
            var title = item.action_display + (item.target_row_id ? ' #' + item.target_row_id : ' (новая)');

            html += '<div class="diff-section">';
            html += '<div class="diff-section-header"><span class="sandbox-action-badge ' + actionClass + '">' + icon + ' ' + esc(title) + '</span></div>';

            if (item.action === 'create' && item.new_data) {
                Object.keys(item.new_data).forEach(function(field) {
                    html += '<div class="diff-field-row">' +
                        '<span class="diff-field-name">' + esc(field) + '</span>' +
                        '<span class="diff-field-new">' + esc(String(item.new_data[field] || '')) + '</span>' +
                        '</div>';
                });
            } else if (item.action === 'update' && item.changes) {
                item.changes.forEach(function(c) {
                    html += '<div class="diff-field-row">' +
                        '<span class="diff-field-name">' + esc(c.field) + '</span>' +
                        '<span class="diff-field-old">' + esc(String(c.old ?? '')) + '</span>' +
                        '<span class="diff-field-new">' + esc(String(c.new ?? '')) + '</span>' +
                        (c.conflict ? '<span class="diff-field-conflict">⚠ Конфликт</span>' : '') +
                        '</div>';
                });
            } else if (item.action === 'delete' && item.deleted_data) {
                html += '<div class="diff-field-row"><span class="diff-field-name" style="color:var(--danger);">Удаляемая строка</span>' +
        ...
        if (currentChangeset && currentChangeset.status === 'review' && _getSbConfig().canApprove) {
            footer.innerHTML = '<button class="btn btn-success btn-sm" onclick="closeModal(\'changesetDiffModal\');approveChangeset();"><i class="fas fa-check"></i> Утвердить</button>' +
        ...
        body.innerHTML = '<div class="alert alert-error">Ошибка: ' + esc(err.message || String(err)) + '</div>';
    });
}
