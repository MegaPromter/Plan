/**
 * Общие JS-утилиты для SPA-страниц (fetchJson, toast, модалки).
 */
...
function _ensureToastContainer() {
  ...
  _toastContainer =
    document.getElementById('toastContainer') || document.getElementById('toast-container');
  if (_toastContainer) return _toastContainer;
...
function createScrollLoader(container, onNearBottom, threshold = 200) {
  ...
  function onScroll() {
    ...
    requestAnimationFrame(() => {
      ...
      let distToBottom;
      if (container === window) {
        distToBottom = document.documentElement.scrollHeight - window.scrollY - window.innerHeight;
      } else {
        distToBottom = container.scrollHeight - container.scrollTop - container.clientHeight;
      }
...
function createLazyRender(opts) {
  ...
  function appendBatch(count) {
    ...
    if (renderedCount < filtered.length) {
      ...
      spinnerTr.innerHTML =
        '<td colspan="' +
        (opts.colSpan || 15) +
        '" class="scroll-spinner"><i class="fas fa-spinner"></i> Загрузка...</td>';
  ...
  function attachScroll() {
    ...
    if (!wrap) return;
    scrollDispose = createScrollLoader(
      wrap,
      function () {
        if (renderedCount < filtered.length) {
...
function updateStatusPanel(opts) {
  ...
  var el;
  el = document.getElementById(p + 'CountAll');
  if (el) el.textContent = total;
  el = document.getElementById(p + 'CountDone');
  if (el) el.textContent = done;
  el = document.getElementById(p + 'CountOverdue');
  if (el) el.textContent = overdue;
  el = document.getElementById(p + 'CountInWork');
  if (el) el.textContent = inwork;

  el = document.getElementById(p + 'BarDone');
  if (el) el.style.width = (done / total) * 100 + '%';
  el = document.getElementById(p + 'BarOverdue');
  if (el) el.style.width = (overdue / total) * 100 + '%';
  el = document.getElementById(p + 'BarInWork');
  if (el) el.style.width = (inwork / total) * 100 + '%';
...
function createMultiFilter(opts) {
  ...
  return {
    ...
    setSelections: function (s) {
      selections = s;
    },
    getActiveDrop: function () {
      return activeDrop;
    },
  };
}
...
function initFadeUp(selector) {
  var els = document.querySelectorAll(
    selector ||
      '.page-content > .section, .page-content > .page-header, .page-content > .filter-bar, .page-content > .data-table-wrap, .page-content > h1, .page-content > .ji-wrap',
  );
...
function empAvatarHtml(fullName) {
  ...
  return (
    ...
    '<span class="emp-avatar-circle" style="background:' +
    color +
    '">' +
...
function initInlineValidation(form, rules) {
  Object.keys(rules).forEach(function (fieldId) {
    ...
    input.addEventListener('blur', function () {
      _validateField(input, rules[fieldId]);
    });
  ...
  function _validateField(input, rule) {
    ...
    if (rule.required && !val) msg = rule.message || 'Обязательное поле';
      ...
      msg = 'Минимум ' + rule.minLength + ' символов';
...
function showShortcutsHelp() {
  ...
  overlay.style.cssText =
    'position:fixed;inset:0;z-index:10001;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);';

  var card = document.createElement('div');
  card.style.cssText =
    'background:var(--surface);border-radius:16px;padding:32px;max-width:480px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.3);max-height:80vh;overflow-y:auto;';
  ...
  header.innerHTML =
    '<h3 style="font-size:18px;font-weight:700;color:var(--text);margin:0;">Горячие клавиши</h3>';
  ...
  var kbdStyle =
    'background:var(--surface2);padding:3px 8px;border-radius:4px;font-size:12px;border:1px solid var(--border);font-family:monospace;';
...
registerShortcut('/', 'Фокус на поиск', function () {
  var search = document.querySelector(
    '.search-input, .searchbar input, input[type="search"], #searchInput, .filter-bar input[type="text"]',
  );
...
registerShortcut('d', 'Удалить выбранное', function () {
  var delBtn = document.querySelector(
    '.btn-delete.active, .toolbar .btn-danger, button[data-action="delete"]',
  );
...
registerShortcut('e', 'Редактировать выбранное', function () {
  var editBtn = document.querySelector(
    '.btn-edit.active, .toolbar .btn-edit, button[data-action="edit"]',
  );
...
function resetBreadcrumbs(pageLabel) {
  updateBreadcrumbs([{ label: 'Главная', url: '/' }, { label: pageLabel }]);
}
