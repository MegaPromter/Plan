...
(function () {
  ...
  function loadDashboard() {
    ...
    fetch('/api/dashboard/?year=' + currentYear + '&month=' + currentMonth)
      ...
      .then(function (data) {
        if (data.error) {
          document.getElementById('dashKPI').innerHTML = emptyStateHtml({
            icon: 'fas fa-exclamation-triangle',
            title: esc(data.error),
          });
          return;
        }
        lastData = data;
        ...
        // Свёрнутые виджеты
        var kpi = data.kpi;
        _updateCollapsedHeader(
          'tasks',
          'fas fa-tasks',
          'Задачи — ' + MONTHS_FULL[currentMonth],
          kpi.done_count + kpi.inwork_count,
          'var(--accent)',
        );
        _updateCollapsedHeader(
          'debts',
        ...
        if (!isLeader) {
          document
            .querySelectorAll(
              '.dash-widget[data-widget="tasks"],.dash-widget[data-widget="debts"],.dash-widget[data-widget="donelate"]',
            )
            .forEach(function (w) {
  ...
  function _updateCollapsedHeader(widgetName, iconCls, label, count, iconColor) {
    ...
    var widget = document.querySelector(
      '.dash-widget[data-widget="' + (wMap[widgetName] || widgetName) + '"]',
    );
    if (!widget) return;
    ...
    if (span) {
      ...
      if (widgetName === 'debts' || widgetName === 'done_late') {
        ...
        exportBtn =
          ' <a href="/api/dashboard/export/?year=' +
          currentYear +
          '&month=' +
          currentMonth +
          '&type=' +
          expType +
          '" class="dash-export-btn" title="Скачать CSV" onclick="event.stopPropagation();">' +
      ...
      span.innerHTML =
        ...
        label +
        (count > 0
          ? ' <span class="an-load-badge" style="font-size:11px;">' + count + '</span>'
          : '') +
        exportBtn;
    }
  }
  ...
  window.dashToggleWidget = function (headerEl) {
    ...
    if (isCollapsed) {
      ...
      if (widget.dataset.loaded === 'false') {
        ...
        body.innerHTML =
          '<div style="padding:12px;color:var(--muted);"><i class="fas fa-spinner fa-spin"></i> Загрузка...</div>';
  ...
  function _fetchWidgetData(widget) {
    ...
    fetch(
      '/api/dashboard/scope/?year=' + currentYear + '&month=' + currentMonth + '&type=' + scopeType,
    )
      .then(function (r) {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
  ...
  function renderMonthChips(years, months) {
    ...
    if (years && years.length > 1) {
      ...
      html +=
        '<button' +
        (idx > 0
          ? ' onclick="dashSetYear(' + years[idx - 1] + ')"'
  ...
  function renderHeroSub(data) {
    ...
    if (kpi.total_debts > 0) {
      parts.push(
        '<span style="background:rgba(239,68,68,0.25);padding:2px 10px;border-radius:8px;font-weight:600;"><i class="fas fa-exclamation-circle"></i> Долги: ' +
          kpi.total_debts +
          '</span>',
      );
    }
    if (kpi.inwork_count > 0) {
      parts.push(
        '<span style="background:rgba(245,158,11,0.25);padding:2px 10px;border-radius:8px;font-weight:600;"><i class="fas fa-clock"></i> В работе: ' +
          kpi.inwork_count +
          '</span>',
      );
    }
  ...
  function _sparklineHtml(months, field, w, h) {
    ...
    return (
      '<svg class="dash-sparkline" width="' +
      w +
      '" height="' +
      h +
      '" viewBox="0 0 ' +
  ...
  function renderKPI(kpi, months, prevKpi) {
    ...
    var cards = [
      ...
      {
        val: fmtHrs(kpi.planned_hours) + ' / ' + fmtHrs(kpi.norm_hours),
        label: 'План / Норма (ч)',
        cls: 'an-val-accent',
      },
      {
        ...
        label: 'Выполнено',
        cls: 'an-val-green',
        extra: _deltaHtml(kpi.done_count, pv.done_count, false),
      },
      { val: kpi.inwork_count, label: 'В работе', cls: 'an-val-accent', action: 'tasks' },
      {
        val: kpi.total_debts,
        label: 'Долги',
        cls: kpi.total_debts > 0 ? 'an-val-red' : 'an-val-green',
        action: 'debts',
        extra: _deltaHtml(kpi.total_debts, pv.total_debts, true),
      },
    ];

    if (kpi.on_time_pct >= 0 && (kpi.total_done || 0) >= 3) {
      cards.push({
        val: fmtPct(kpi.on_time_pct),
        label: '% в срок',
        cls:
          kpi.on_time_pct >= 90
            ? 'an-val-green'
            : kpi.on_time_pct >= 70
              ? 'an-val-yellow'
              : 'an-val-red',
        extra: _deltaHtml(kpi.on_time_pct, pv.on_time_pct, false),
      });
    }
    ...
    cards.forEach(function (c) {
      ...
      var clickAttr = c.action
        ? ' onclick="dashOpenWidget(\'' + c.action + '\')" style="cursor:pointer;"'
        : '';
  ...
  function renderMyKPI(myKpi) {
    ...
    var cards = [
      ...
      {
        val: myKpi.total_debts,
        label: 'Мои долги',
        cls: myKpi.total_debts > 0 ? 'an-val-red' : 'an-val-green',
      },
    ];
  ...
  function renderTeam(team) {
    ...
    if (widget) {
      ...
      _updateWidgetHeader(
        widget,
        '<i class="fas fa-users" style="color:var(--accent);margin-right:6px;"></i>Команда (' +
          team.total_employees +
          ') &nbsp; <span class="an-load-badge ' +
          loadBadgeCls(team.avg_load_pct) +
          '">' +
          fmtPct(team.avg_load_pct) +
          ' ср.загрузка</span>' +
          (team.total_overdue > 0
            ? ' &nbsp; <span class="an-load-badge over">' + team.total_overdue + ' просроч.</span>'
            : ''),
      );
    }
    ...
    html +=
      '<input type="text" class="dash-team-search" id="dashTeamSearch" placeholder="Поиск по ФИО..." oninput="dashFilterTeam(this.value)">';
    html += '</div>';
    ...
    if (deptWithout.length > 0) {
      ...
      html +=
        '<button class="dash-show-all-btn" onclick="dashToggleAllDepts(this)"><i class="fas fa-chevron-down"></i> Показать ещё ' +
        deptWithout.length +
        ' отд. (без просрочек)</button>';
  ...
  function _renderDeptBlock(dept) {
    ...
    if (dept.overdue_count > 0) {
      html +=
        ' &middot; <span class="an-load-badge over" style="font-size:11px;">' +
        dept.overdue_count +
        ' просроч.</span>';
    }
    html += '</span></div>';
  ...
  window.dashToggleAllDepts = function (btn) {
    ...
    btn.innerHTML = isHidden
      ...
      : '<i class="fas fa-chevron-down"></i> Показать ещё ' +
        body.querySelectorAll('.dash-dept').length +
        ' отд. (без просрочек)';
  };
  ...
  function _renderEmployeeDropdown(e) {
    ...
    html +=
      '<span class="dash-emp-load"><span class="an-load-badge ' +
      badgeCls +
      '">' +
      fmtPct(e.load_pct) +
      '</span></span>';
  ...
  function _renderCompactList(tasks) {
    ...
    tasks.forEach(function (t) {
      ...
      var badge = '';
      if (t.days_overdue)
        badge = '<span class="dash-days-badge dash-days-overdue">' + t.days_overdue + ' дн.</span>';
      else if (t.days_left !== undefined && t.days_left >= 0)
        badge = '<span class="dash-days-badge dash-days-left">' + t.days_left + ' дн.</span>';
      else if (t.days_late)
        badge = '<span class="dash-days-badge dash-days-late">+' + t.days_late + ' дн.</span>';
      html += '<li>';
      html += '<span class="dcl-name">' + esc(t.work_name) + '</span>';
      if (t.work_designation)
        html += '<span class="dcl-designation">' + esc(t.work_designation) + '</span>';
  ...
  function renderTasks(tasks, showExecutor) {
    ...
    if (widget)
      _updateWidgetHeader(
        widget,
        '<i class="fas fa-tasks" style="color:var(--accent);margin-right:6px;"></i>Задачи — ' +
    ...
    if (!tasks || !tasks.length) {
      el.innerHTML = emptyStateHtml({
        ...
        title: 'Нет задач на этот месяц',
        desc: 'Все задачи выполнены или ещё не запланированы',
      });
      return;
    }
  ...
  function renderDebts(debts, showExecutor) {
    ...
    if (widget) {
      ...
      _updateWidgetHeader(
        widget,
        '<i class="fas fa-exclamation-triangle" style="color:var(--danger);margin-right:6px;"></i>Долги — просроченные задачи (' +
          debts.length +
          ')',
      );
    }
  ...
  function renderDoneLate(items, showExecutor) {
    ...
    if (widget) {
      ...
      _updateWidgetHeader(
        widget,
        '<i class="fas fa-clock" style="color:#d97706;margin-right:6px;"></i>Выполнены с просрочкой (' +
          items.length +
          ')',
      );
    }
  ...
  function _renderTasksTable(tasks, showExecutor) {
    ...
    tasks.forEach(function (t) {
      ...
      if (t.work_designation)
        html += '<span class="dcl-designation">' + esc(t.work_designation) + '</span>';
      if (t.project_name) html += '<span class="dcl-project">' + esc(t.project_name) + '</span>';
      if (showExecutor && t.executor_name)
        html +=
          '<span class="dcl-executor">' +
          avatarHtml(t.executor_name, 'sm') +
          ' ' +
          esc(t.executor_name) +
          '</span>';
      html += '<span class="dcl-date">' + fmtDate(t.date_end) + '</span>';
  ...
  window.dashToggleEmp = function (headerEl) {
    ...
    if (!isOpen && body.dataset.loaded === 'false') {
      ...
      body.innerHTML =
        '<div style="padding:8px;color:var(--muted);"><i class="fas fa-spinner fa-spin"></i> Загрузка...</div>';
      fetch('/api/dashboard/employee/' + empId + '/?year=' + currentYear + '&month=' + currentMonth)
        .then(function (r) {
          if (!r.ok) throw new Error('HTTP ' + r.status);
          return r.json();
        ...
        .then(function (data) {
          ...
          if (data.tasks && data.tasks.length) {
            html +=
              '<div class="dash-emp-label"><i class="fas fa-tasks"></i> ' +
              MONTHS_FULL[currentMonth] +
              '</div>';
          ...
          if (data.debts && data.debts.length) {
            html +=
              '<div class="dash-emp-label debt"><i class="fas fa-exclamation-triangle"></i> Долги</div>';
  ...
  function initWidgetDrag() {
    ...
    grid.addEventListener('dragleave', function (e) {
      var t = e.target.closest('.dash-widget');
      if (t) t.classList.remove('drag-over-widget');
    });
