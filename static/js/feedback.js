...
(function () {
  ...
  function renderFilterChips() {
    ...
    html +=
      '<button class="fb-chip' +
      (!filterStatus ? ' active' : '') +
      '" onclick="fbSetStatus(\'\')">Все</button>';
    Object.keys(STATUSES).forEach(function (k) {
      html +=
        '<button class="fb-chip fb-chip-' +
        k +
        (filterStatus === k ? ' active' : '') +
        '" onclick="fbSetStatus(\'' +
        k +
        '\')">' +
        STATUSES[k] +
    ...
    html +=
      ...
      '" onclick="fbSetCategory(\'\')">Все</button>';
    Object.keys(CATEGORIES).forEach(function (k) {
      html +=
        '<button class="fb-chip fb-chip-' +
        k +
        (filterCategory === k ? ' active' : '') +
        '" onclick="fbSetCategory(\'' +
        k +
        '\')">' +
        CATEGORIES[k] +
        '</button>';
  ...
  window.fbSetStatus = function (s) {
    filterStatus = s;
    renderFilterChips();
    fbLoad();
  };
  window.fbSetCategory = function (c) {
    filterCategory = c;
    renderFilterChips();
    fbLoad();
  };
  ...
  function renderList(items) {
    ...
    if (!items.length) {
      el.innerHTML = emptyStateHtml({
        ...
        title: 'Нет замечаний',
        desc: 'Здесь будут отображаться замечания и обращения',
        action:
          '<button class="btn btn-primary btn-sm" onclick="fbOpenCreate()"><i class="fas fa-plus"></i> Создать обращение</button>',
      });
      return;
    }
    var html = '';
    items.forEach(function (fb) {
      ...
      html +=
        '<span class="fb-cat fb-cat-' + fb.category + '">' + esc(fb.category_display) + '</span>';
      html +=
        '<span class="fb-meta">' + esc(fb.user_name) + ' · ' + fmtDate(fb.created_at) + '</span>';
      html +=
        '<span class="fb-status fb-status-' + fb.status + '">' + esc(fb.status_display) + '</span>';
      html += '</div>';
      html += '<div class="fb-text">' + esc(fb.text) + '</div>';
      var allScreens = (fb.screenshots || []).map(function (s) {
        return s.url || s;
      });
      ...
      if (fb.admin_comment) {
        html +=
          '<div class="fb-admin-comment"><strong>Ответ:</strong> ' +
          esc(fb.admin_comment) +
          '</div>';
      }
      ...
      if (hasActions) {
        ...
        if (fb.is_own) {
          html +=
            '<button class="btn btn-outline btn-xs" onclick=\'fbOpenEdit(' +
            JSON.stringify({
              id: fb.id,
        ...
        if (isAdmin) {
          html +=
            '<button class="btn btn-outline btn-xs" onclick="fbOpenStatus(' +
            fb.id +
            ",'" +
            fb.status +
            "','" +
            esc(fb.admin_comment).replace(/'/g, "\\'") +
            '\')"><i class="fas fa-edit"></i> Статус</button>';
          html +=
            '<button class="btn btn-danger btn-xs" onclick="fbDelete(' +
            fb.id +
  ...
  window.fbOpenEdit = function (data) {
    ...
    if (existingScreens.length > 0) {
      existingHtml =
        '<label>Текущие скриншоты</label><div class="fb-existing-screens" id="fbExistingScreens">';
      existingScreens.forEach(function (s) {
        existingHtml += '<span class="fb-preview-item" data-att-id="' + s.id + '">';
        existingHtml += '<img src="' + esc(s.url) + '">';
        existingHtml +=
          '<span class="fb-dropzone-remove" onclick="event.stopPropagation(); fbDeleteAttachment(' +
          s.id +
          ',this)"><i class="fas fa-times"></i></span>';
        existingHtml += '</span>';
      });
      existingHtml += '</div>';
    }
    ...
    body.innerHTML =
      '<label>Категория</label><select id="fbEditCat">' +
      opts +
      '</select>' +
      '<label>Текст</label><textarea id="fbEditText">' +
      esc(currentText) +
      '</textarea>' +
      existingHtml +
    ...
    openModal({
      ...
      footer:
        '<button class="btn btn-outline" onclick="closeAllModals()">Отмена</button>' +
        '<button class="btn btn-primary" onclick="fbSubmitEdit(' +
        id +
        ')">Сохранить</button>',
    });
  ...
  window.fbOpenStatus = function (id, currentStatus, currentComment) {
    ...
    Object.keys(STATUSES).forEach(function (k) {
      opts +=
        ...
        k +
        '"' +
        (k === currentStatus ? ' selected' : '') +
        '>' +
        STATUSES[k] +
        '</option>';
    });
    body.innerHTML =
      '<label>Статус</label><select id="fbEditStatus">' +
      opts +
      '</select>' +
      '<label>Комментарий администратора</label>' +
      '<textarea id="fbEditComment">' +
      esc(currentComment) +
      '</textarea>';
    ...
    openModal({
      ...
      footer:
        '<button class="btn btn-outline" onclick="closeAllModals()">Отмена</button>' +
        '<button class="btn btn-primary" onclick="fbSubmitStatus(' +
        id +
        ')">Сохранить</button>',
    });
  };
  ...
  function setupDropzone(dropzoneId, fileInputId, previewId) {
    ...
    function renderPreviews() {
      ...
      files.forEach(function (f, idx) {
        ...
        html +=
          '<span class="fb-dropzone-remove" onclick="event.stopPropagation(); event.preventDefault(); fbRemoveFile(\'' +
          dropzoneId +
          "'," +
          idx +
