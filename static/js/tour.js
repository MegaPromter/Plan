...
(function () {
  ...
  var STEPS = [
    ...
    },
    {
      ...
      adminOnly: true,
      descNonAdmin:
        'Производственный календарь задаёт норму рабочих часов по месяцам. Эти данные используются для расчёта загрузки и поиска перегрузок. Управление доступно администраторам.',
    },
    {
      id: 'roles',
      stepNum: 16,
  ...
  function clearTourUI() {
    ...
    pulsing.forEach(function (el) {
      el.classList.remove('tour-sidebar-pulse');
    });
  ...
  function buildProgress(idx) {
    ...
    return (
      '<div class="tour-progress"><div class="tour-progress-bar" style="width:' +
      pct +
      '%"></div></div>'
    );
  }
  ...
  function drawSidebarArrow(tipEl, step) {
    ...
    svg.style.cssText =
      'position:fixed;top:0;left:0;width:100%;height:100%;z-index:99993;pointer-events:none';
  ...
  function showStep(idx) {
    ...
    if (step.adminOnly && !isAdmin()) {
      ...
      adminNote =
        '<div class="tour-admin-note"><i class="fas fa-lock"></i> Этот шаг выполняет администратор</div>';
    }
  ...
  function showSpotlight(el, step, idx, desc, adminNote) {
    ...
    setTimeout(function () {
      ...
      tip.innerHTML =
        ...
        '<div class="tour-tooltip-icon"><i class="fas ' +
        step.icon +
        '"></i></div>' +
        '<div class="tour-tooltip-title">' +
        step.title +
        '</div>' +
        '<div class="tour-tooltip-counter">Шаг ' +
        (idx + 1) +
        ' из ' +
        TOTAL +
        '</div>' +
  ...
  function showInfoCard(step, idx, desc, adminNote) {
    ...
    backdrop.innerHTML =
      ...
      buildProgress(idx) +
      '<div class="tour-info-icon"><i class="fas ' +
      step.icon +
      '"></i></div>' +
      '<div class="tour-info-title">' +
      step.title +
      '</div>' +
      '<div class="tour-info-counter">Шаг ' +
      (idx + 1) +
      ' из ' +
      TOTAL +
      '</div>' +
  ...
  function showProgressWidget(stepIdx) {
    ...
    widget.innerHTML =
      ...
      '<div class="tour-progress-widget-status">\u041E\u0431\u0437\u043E\u0440 \u0441\u0438\u0441\u0442\u0435\u043C\u044B: ' +
      stepIdx +
      ' \u0438\u0437 ' +
      TOTAL +
      '</div>' +
      '<div class="tour-progress-widget-bar"><div class="tour-progress-widget-bar-fill" style="width:' +
      pct +
      '%"></div></div>' +
