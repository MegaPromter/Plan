...
function initColumnPicker(opts) {
    ...
    columns.forEach(function(col) {
        state[col.key] = saved.hasOwnProperty(col.key) ? saved[col.key] : (col.default !== false);
    });

    // Create button
    var container = typeof btnContainer === 'string' ? document.querySelector(btnContainer) : btnContainer;
    if (!container) return;
    ...
    dropdown.style.cssText = 'display:none;position:absolute;top:100%;right:0;z-index:100;background:var(--surface);border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.12);padding:8px 0;min-width:220px;max-height:400px;overflow-y:auto;';

    columns.forEach(function(col) {
        var item = document.createElement('label');
        item.style.cssText = 'display:flex;align-items:center;gap:10px;padding:8px 16px;cursor:pointer;font-size:14px;color:var(--text);transition:background 0.1s;';
    ...
    resetLink.onclick = function(e) {
        e.preventDefault();
        columns.forEach(function(col) { state[col.key] = col.default !== false; });
    ...
    function applyVisibility() {
        ...
        columns.forEach(function(col, colIdx) {
            ...
            if (col.colIndex !== undefined) {
                ...
                table.querySelectorAll('td:nth-child(' + ci + '), th:nth-child(' + ci + ')').forEach(function(cell) {
                    cell.style.display = display;
                });
            }
        });
    ...
    return { applyVisibility: applyVisibility, getState: function() { return state; } };
}
