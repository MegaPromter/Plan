...
(function () {
  ...
  var PAGES = [
    ...
    },
    {
      ...
      keywords: 'производство production',
    },
    {
      title: 'Сводное планирование',
      icon: 'fas fa-tasks',
      url: '/works/plan/',
      keywords: 'план отчёт plan report',
    },
    {
      title: 'Журнал извещений',
      icon: 'fas fa-envelope',
      url: '/works/notices/',
      keywords: 'извещения notices',
    },
    {
      title: 'Аналитика',
      icon: 'fas fa-chart-bar',
      url: '/works/analytics/',
      keywords: 'аналитика analytics графики',
    },
    {
    ...
    },
    {
      ...
      icon: 'fas fa-users',
      url: '/employees/',
      keywords: 'сотрудники employees люди',
    },
    {
      title: 'План отпусков',
      icon: 'fas fa-calendar-alt',
      url: '/employees/vacation-plan/',
      keywords: 'отпуск vacation',
    },
    {
      title: 'План командировок',
      icon: 'fas fa-plane',
      url: '/works/business-trips/',
      keywords: 'командировки trips',
    },
    {
      title: 'Замечания и предложения',
      icon: 'fas fa-lightbulb',
      url: '/works/feedback/',
      keywords: 'обратная связь feedback',
    },
    {
      title: 'Журнал аудита',
      icon: 'fas fa-shield-alt',
      url: '/works/audit-log/',
      keywords: 'аудит audit лог',
    },
    {
      title: 'Профиль',
      icon: 'fas fa-cog',
      url: '/accounts/profile/',
      keywords: 'настройки profile settings',
    },
  ];
      ...
      actions.push({
        ...
        action: function () {
          ...
          if (btn) btn.click();
            ...
            showToast('Кнопка создания задачи не найдена', 'warning');
        },
      });
    }
      ...
      actions.push({
        ...
        action: function () {
          var btn = document.querySelector(
            '[data-action="export"], .btn-export, #exportBtn, .export-btn',
          );
          if (btn) btn.click();
          else if (typeof showToast === 'function')
            showToast('Кнопка экспорта не найдена', 'warning');
        },
      });
    }
  ...
  function open() {
    ...
    function render(q) {
      ...
      if (!q) {
        ...
        filteredPages = PAGES.filter(function (p) {
          return scoreItem(p, q) > 0;
        }).sort(function (a, b) {
          return scoreItem(b, q) - scoreItem(a, q);
        });
        filteredActions = actions
          .filter(function (a) {
            return scoreItem(a, q) > 0;
          })
          .sort(function (a, b) {
            return scoreItem(b, q) - scoreItem(a, q);
          });
      ...
      if (filteredActions.length) {
        ...
        filteredActions.forEach(function (item) {
          ...
          html +=
            ...
            (idx === 0 && !filteredPages.length ? ' active' : '') +
            '" data-idx="' +
            idx +
            '">' +
            '<i class="' +
