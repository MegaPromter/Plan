...
function openOverlapsModal(cfg) {
  ...
  var sortedDepts = Object.keys(deptGroups).sort(function(a, b) { return a.localeCompare(b, 'ru'); });
  ...
  var html = '<div class="overlap-modal-content">' +
  ...
  sortedDepts.forEach(function(dept) {
    html += '<div class="overlap-emp-group" data-dept="' + escapeHtml(dept) + '">' +
      '<label class="overlap-dept-header"><input type="checkbox" class="ov-dept-cb" data-dept="' + escapeHtml(dept) + '" onchange="ovToggleDept(this)"> <strong>' + escapeHtml(dept) + '</strong> <span class="text-muted">(' + deptGroups[dept].length + ')</span></label>';
    deptGroups[dept].forEach(function(emp) {
      html += '<label class="overlap-emp-item"><input type="checkbox" class="ov-emp-cb" value="' + emp.id + '" data-dept="' + escapeHtml(dept) + '" onchange="ovUpdateCount()"> ' + escapeHtml(emp.value) + '</label>';
    });
    html += '</div>';
  });
  ...
  html += '</div></div></div>' +
    ...
    '<input type="date" id="ovDateFrom" value="' + yearStart + '" class="form-input" style="width:150px;">' +
    '<span>—</span>' +
    '<input type="date" id="ovDateTo" value="' + yearEnd + '" class="form-input" style="width:150px;">' +
    '</div></div>' +
    '<div class="overlap-form-row">' +
    '<label class="overlap-check-label"><input type="checkbox" id="ovCrossCheck" checked> ' + crossLabel + '</label>' +
    '</div>' +
    '<div style="text-align:right;margin-top:12px;">' +
    '<button class="btn btn-primary" onclick="runOverlapCheck(\'' + ctx + '\')"><i class="fas fa-search"></i> Проверить</button>' +
    '</div></div>' +
...
function ovSelectAll() {
  ...
  document.querySelectorAll('#ovEmpList .ov-dept-cb').forEach(function(cb) { cb.checked = true; });
  ovUpdateCount();
}

function ovDeselectAll() {
  document.querySelectorAll('#ovEmpList .ov-emp-cb').forEach(function(cb) { cb.checked = false; });
  document.querySelectorAll('#ovEmpList .ov-dept-cb').forEach(function(cb) { cb.checked = false; });
  ovUpdateCount();
}
...
function ovToggleDept(deptCb) {
  ...
  document.querySelectorAll('#ovEmpList .ov-emp-cb[data-dept="' + dept + '"]').forEach(function(cb) {
    cb.checked = checked;
  });
  ovUpdateCount();
}
...
function runOverlapCheck(ctx) {
  ...
  if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Проверяю...'; }
  ...
  .then(function(data) {
    if (data.error) {
      ...
      if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-search"></i> Проверить'; }
      return;
    }
...
function renderOverlapResults(data, dateFrom, dateTo) {
  ...
  if (s.total_overlaps === 0) {
    html += '<div class="overlap-no-result"><i class="fas fa-check-circle" style="color:var(--success);font-size:24px;"></i><br>Пересечений не найдено</div>';
    el.innerHTML = html + '</div>';
    return;
  }
  html += '<div class="overlap-summary-stats">' +
    '<span class="overlap-stat"><i class="fas fa-exclamation-triangle" style="color:var(--danger);"></i> Пересечений: <strong>' + s.total_overlaps + '</strong></span>' +
    '<span class="overlap-stat"><i class="fas fa-users"></i> Сотрудников затронуто: <strong>' + s.employees_involved + '</strong></span>' +
    '<span class="overlap-stat"><i class="fas fa-calendar-day"></i> Макс. дней пересечения: <strong>' + s.max_overlap_days + '</strong></span>' +
    '</div></div>';

  // Таблица пересечений
  html += '<div class="overlap-table-wrap"><table class="overlap-table">' +
    '<thead><tr><th>Период пересечения</th><th>Дней</th><th>Сотрудники</th></tr></thead><tbody>';
  data.overlaps.forEach(function(ov) {
    html += '<tr><td class="ov-dates">' + formatDate(ov.overlap_start) + ' — ' + formatDate(ov.overlap_end) + '</td>' +
      '<td class="ov-days">' + ov.duration_days + '</td><td>';
    ov.employees.forEach(function(emp) {
      ...
      html += '<div class="ov-emp-row"><span class="ov-emp-name">' + escapeHtml(emp.employee_name) + '</span> ' +
        '<span class="ov-badge ' + badge + '">' + typeLabel + '</span> ' +
        '<span class="ov-emp-detail">' + escapeHtml(emp.detail) + ' (' + formatDate(emp.period_start) + ' — ' + formatDate(emp.period_end) + ')</span></div>';
    });
    html += '</td></tr>';
  });
...
function _buildTimeline(timeline, overlaps, dateFrom, dateTo) {
  ...
  var html = '<div class="overlap-timeline"><h4 style="margin:16px 0 8px;">Таймлайн отсутствий</h4>';
  ...
  while (cur <= maxD) {
    ...
    html += '<span class="ov-tl-month" style="left:' + left.toFixed(1) + '%">' + monthNames[cur.getMonth()] + '</span>';
    cur.setMonth(cur.getMonth() + 1);
  }
  html += '</div>';

  timeline.forEach(function(row) {
    html += '<div class="ov-tl-row">' +
      '<div class="ov-tl-name" title="' + escapeHtml(row.employee_name) + '">' + escapeHtml(row.employee_name) + ' <span class="text-muted">(' + escapeHtml(row.dept) + ')</span></div>' +
    ...
    row.periods.forEach(function(p) {
      ...
      var tip = (p.type === 'vacation' ? 'Отпуск: ' : 'Командировка: ') + p.detail + ' (' + formatDate(p.start) + ' — ' + formatDate(p.end) + ')';
      html += '<div class="ov-tl-bar ' + cls + '" style="left:' + left.toFixed(2) + '%;width:' + width.toFixed(2) + '%;" title="' + escapeHtml(tip) + '"></div>';
    });
    html += '</div></div>';
  });
