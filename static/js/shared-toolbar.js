...
function initDeptFilter(cfg) {
  ...
  if (mode === 'dropdown') {
    ...
    window[fnPrefix + 'ToggleDeptMenu'] = function() {
      ...
      if (menu.classList.contains('open')) {
        ...
        if (inp) { inp.value = ''; _deptFilterList(prefix, depts); inp.focus(); }
      }
    };
    ...
    window[fnPrefix + 'ToggleDeptItem'] = function(cb) {
      ...
      var newSel = new Set();
      checked.forEach(function(c) { newSel.add(c.value); });
      setSel(newSel);
      _deptUpdateUI(wrap, prefix, mode, multi, depts, newSel);
      onApply();
    };
    window[fnPrefix + 'ClearDeptFilter'] = function() {
      document.querySelectorAll('#' + prefix + 'DeptMenuItems .dept-cb').forEach(function(c) { c.checked = false; });
      setSel(new Set());
  ...
  return { mode: mode, refresh: function() {
    var s = getSel();
    if (mode === 'dropdown') {
...
function _deptRenderDropdown(wrap, prefix, depts, sel, multi) {
  ...
  var label = hasFilter
    ? (sel.size === 1 ? [...sel][0] : sel.size + ' отд.')
    : 'Все отделы';
  var fnPfx = prefix || 'shared';
  var inputType = multi ? 'checkbox' : 'radio';
  var html = '<div class="dept-dropdown" id="' + prefix + 'DeptDropdown">' +
    '<button class="dept-trigger' + (hasFilter ? ' has-filter' : '') + '" id="' + prefix + 'DeptTrigger" onclick="' + fnPfx + 'ToggleDeptMenu()">' +
    '<i class="fas fa-building" style="font-size:12px;opacity:0.6"></i> ' +
    '<span id="' + prefix + 'DeptLabel">' + escapeHtml(label) + '</span> ' +
    ...
    '</button>' +
    '<div class="dept-menu" id="' + prefix + 'DeptMenu">' +
    '<div class="dept-menu-search"><input type="text" placeholder="Поиск отдела..." id="' + prefix + 'DeptSearch" oninput="' + fnPfx + 'FilterDeptList()"></div>' +
    '<div class="dept-menu-items" id="' + prefix + 'DeptMenuItems">';
  depts.forEach(function(d) {
    var checked = sel.has(d) ? ' checked' : '';
    html += '<label class="dept-menu-item" data-dept="' + escapeHtml(d) + '">' +
      '<input type="' + inputType + '" name="' + prefix + 'Dept" class="dept-cb" value="' + escapeHtml(d) + '"' + checked + ' onchange="' + fnPfx + 'ToggleDeptItem(this)">' +
      '<span>' + escapeHtml(d) + '</span></label>';
  });
  html += '</div>' +
    '<div class="dept-menu-footer">' +
    '<button class="dm-clear" onclick="' + fnPfx + 'ClearDeptFilter()">Сбросить</button>' +
    '</div></div></div>';
  wrap.innerHTML = html;
}
...
function _deptRenderChips(wrap, prefix, depts, sel) {
  var fnPfx = prefix || 'shared';
  var html = '<span class="pp-dept-chip' + (sel.size === 0 ? ' active' : '') + '" onclick="' + fnPfx + 'SelectDept(null)">Все</span>';
  depts.forEach(function(d) {
    html += '<span class="pp-dept-chip' + (sel.has(d) ? ' active' : '') + '" data-dept="' + escapeHtml(d) + '" onclick="' + fnPfx + 'SelectDept(this.dataset.dept)">' + escapeHtml(d) + '</span>';
  });
  wrap.innerHTML = html;
}
