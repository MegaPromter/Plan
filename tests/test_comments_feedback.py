...
def admin_user_obj(db, dept):
    ...
    Employee.objects.create(user=user, last_name='Админ2', role='admin', department=dept)
    return user
...
def user2_client(db, dept2):
    ...
    Employee.objects.create(user=user, last_name='Другой', role='user', department=dept2)
    c = Client()
...
def work(db, dept):
    return Work.objects.create(
        work_name='Тестовая задача', show_in_plan=True, department=dept,
    )
...
def work_dept2(db, dept2):
    return Work.objects.create(
        work_name='Задача отдела 202', show_in_plan=True, department=dept2,
    )
...
class TestCommentsVisibility:
    ...
    def test_user_cannot_delete_comment_other_dept(self, user2_client, work, admin_user_obj):
        """Пользователь НЕ может удалить комментарий из чужого отдела."""
        comment = WorkComment.objects.create(work=work, author=admin_user_obj, text='Чужой')
...
class TestCommentsAuditLog:
    ...
    def test_delete_creates_audit_log(self, admin_client, work, admin_user_obj):
        ...
        comment = WorkComment.objects.create(work=work, author=admin_user_obj, text='Удалить меня')
...
class TestFeedbackFileValidation:
    ...
    def test_rejects_oversized_file(self, user_client):
        ...
        big_file = SimpleUploadedFile(
            'big.png', b'\x89PNG' + b'\x00' * (6 * 1024 * 1024),
            content_type='image/png',
        )
        r = user_client.post('/api/feedback/', {
            'text': 'Баг', 'category': 'bug', 'screenshot': big_file,
        })
    ...
    def test_rejects_non_image_file(self, user_client):
        ...
        exe_file = SimpleUploadedFile(
            'hack.exe', b'MZ\x00\x00',
            content_type='application/x-msdownload',
        )
        r = user_client.post('/api/feedback/', {
            'text': 'Баг', 'category': 'bug', 'screenshot': exe_file,
        })
        assert r.status_code == 400
    ...
    def test_accepts_valid_image(self, user_client):
        ...
        r = user_client.post('/api/feedback/', {
            'text': 'Баг', 'category': 'bug', 'screenshot': img,
        })
...
class TestEnterpriseDependencyIsolation:
    ...
    def writer_client(self, db, dept):
        ...
        Employee.objects.create(user=user, last_name='Писатель', role='dept_head', department=dept)
        c = Client()
    ...
    def test_cross_project_dependency_rejected(self, writer_client, two_projects_with_stages):
