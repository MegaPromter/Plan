...
def task_type_dir(db):
    ...
    return Directory.objects.create(
        dir_type="task_type", value="Выпуск нового документа"
    )
...
class TestProductionPlanVisibility:
    ...
    def test_dept_head_sees_own_dept_only(
        self,
        dept_head_user,
        pp_work_dept1,
        pp_work_dept2,
    ):
        """Начальник отдела по умолчанию видит только свой отдел."""
        client = Client()
        ...
        ids = {w["id"] for w in data}
        assert pp_work_dept1.id in ids
        assert pp_work_dept2.id not in ids

    def test_dept_head_sees_all_with_scope(
        self,
        dept_head_user,
        pp_work_dept1,
        pp_work_dept2,
    ):
        """scope=all снимает фильтр по роли — виден весь проект."""
        client = Client()
        client.force_login(dept_head_user)
        resp = client.get("/api/production_plan/?scope=all")
        assert resp.status_code == 200
        data = resp.json()
        ids = {w["id"] for w in data}
        assert pp_work_dept1.id in ids
    ...
    def test_regular_user_sees_own_dept(
        self,
        regular_user,
        pp_work_dept1,
        pp_work_dept2,
    ):
        """Обычный пользователь видит строки своего отдела (сектор не задан)."""
        client = Client()
    ...
    def test_other_dept_head_sees_own_dept(
        self,
        dept_head_other,
        pp_work_dept1,
        pp_work_dept2,
    ):
        """Начальник чужого отдела видит только свои записи."""
        client = Client()
        ...
        data = resp.json()
        ids = {w["id"] for w in data}
        assert pp_work_dept1.id not in ids
        assert pp_work_dept2.id in ids
