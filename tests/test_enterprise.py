...
class TestPortfolio:
    ...
    def test_update_status(self, writer_client, project):
        r = _put(
            writer_client, f"{API}/portfolio/{project.id}/", {"status": "suspended"}
        )
        assert r.status_code == 200
    ...
    def test_priority(self, admin_client, project):
        r = _post(
            admin_client,
            f"{API}/portfolio/{project.id}/priority/",
            {
                "priority_number": 1,
...
class TestGG:
    ...
    def test_create_milestone(self, writer_client, project):
        ...
        r = _post(
            writer_client,
            f"{API}/gg/{project.id}/milestones/",
            {
                "name": "КТ-1",
                "date": "2026-05-01",
            },
...
class TestCrossSchedule:
    ...
    def test_create_stage_and_milestone(self, writer_client, project):
        ...
        r = _post(
            writer_client,
            f"{API}/cross/{project.id}/milestones/",
            {
                "name": "M1",
                "date": "2026-06-01",
...
class TestBaseline:
    def test_create_baseline(self, writer_client, project):
        ...
        r = _post(
            writer_client,
            f"{API}/cross/{project.id}/baselines/",
            {
                "comment": "Версия 1",
            },
        )
    ...
    def test_view_baseline_detail(self, writer_client, project):
        _post(writer_client, f"{API}/cross/{project.id}/")
        r = _post(
            writer_client, f"{API}/cross/{project.id}/baselines/", {"comment": "v1"}
        )
...
class TestCapacity:
    ...
    def test_capacity_returns_data(self, reader_client, dept):
        ...
        dept_names += [
            d["department_name"] for d in data.get("no_center_departments", [])
        ]
