...
def task_a(db, dept):
    return Work.objects.create(
        show_in_plan=True, work_name='Задача A',
        department=dept,
...
def task_b(db, dept):
    return Work.objects.create(
        show_in_plan=True, work_name='Задача B',
        department=dept,
        date_start=date(2026, 1, 5), date_end=date(2026, 1, 15),
    )
...
def task_c(db, dept):
    return Work.objects.create(
        show_in_plan=True, work_name='Задача C',
        department=dept,
        date_start=date(2026, 1, 20), date_end=date(2026, 1, 30),
    )
...
def pp_task(db, dept):
    ...
    return Work.objects.create(
        ...
        work_name='PP Only', department=dept,
        pp_project=pp_proj,
    )
...
class TestCycleDetection:
    ...
    def test_transitive_cycle(self, writer_client, task_a, task_b, task_c):
        ...
        TaskDependency.objects.create(
            predecessor=task_b, successor=task_c,
        )
        # C → A — цикл
        resp = writer_client.post(
            f'/api/tasks/{task_a.id}/dependencies/',
...
class TestAlignDates:
    ...
    def test_align_pp_rejected(self, writer_client, pp_task, task_a):
        ...
        TaskDependency.objects.create(
            predecessor=task_a, successor=pp_task,
        )
        resp = writer_client.post(
