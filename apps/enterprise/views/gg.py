...
def _serialize_gg(schedule):
    ...
    deps = list(
        GGStageDependency.objects.filter(
...
class GGStageCreateView(WriterRequiredJsonMixin, View):
    ...
    def post(self, request, project_id):
        ...
        max_order = (
            schedule.stages.order_by("-order").values_list("order", flat=True).first()
            or 0
        )
...
class GGStageDependencyCreateView(WriterRequiredJsonMixin, View):
    ...
    def post(self, request):
        ...
        if not pred_id or not succ_id:
            return JsonResponse(
                {"error": "predecessor_id и successor_id обязательны"},
                status=400,
            )
        if pred_id == succ_id:
        ...
        pred = (
            GGStage.objects.filter(pk=pred_id)
        ...
        succ = (
            GGStage.objects.filter(pk=succ_id)
            .values_list("schedule_id", flat=True)
            .first()
        )
        if succ is None:
        ...
        if pred != succ:
            return JsonResponse(
                {"error": "Этапы должны принадлежать одному генеральному графику"},
                status=400,
            )
