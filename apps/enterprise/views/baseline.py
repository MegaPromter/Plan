    ...
    d = {
        ...
        "created_by": (
            (
                ...
                or (
                    snapshot.created_by.user.username
                    if snapshot.created_by and snapshot.created_by.user_id
                    else ""
                )
            )
    ...
    def post(self, request, project_id):
        ...
        with transaction.atomic():
            try:
                ...
                return JsonResponse(
                    {"error": "Сквозной график не найден"},
                    status=404,
                )

            # Атомарный инкремент версии
            last_version = (
                cs.baselines.order_by("-version")
                .values_list(
                    "version",
                    flat=True,
                )
...
class ScenarioListView(LoginRequiredJsonMixin, View):
    ...
    def get(self, request):
        qs = Scenario.objects.select_related("project", "created_by").order_by(
            "-created_at"
        )
            ...
            if f in data:
                if f == "status":
                    ...
                    if data[f] not in valid:
                        return JsonResponse(
                            {"error": f"Недопустимый статус: {data[f]}"},
                            status=400,
                        )
                setattr(sc, f, data[f])
