...
class CrossSchedule(models.Model):
    ...
    Жизненный цикл сквозного графика управляется полем edit_owner:
    - 'cross'  → Редактирование у менеджера сквозного графика (этапы, вехи).
                 Подразделения видят, но не могут изменять ПП-записи в разрезе графика.
    - 'pp'     → Редактирование передано подразделениям через ПП.
                 Каждый отдел вносит свои работы в рамках выделенных этапов.
    - 'locked' → График заблокирован (финальная версия / снимок).
                 Никаких изменений не допускается; нужно создать новую итерацию (version + 1).

    Гранулярность (granularity) определяет детализацию блокировки:
    - 'whole'    → Весь график блокируется/разблокируется как единое целое.
    - 'per_dept' → Блокировка управляется пообделённо через CrossScheduleDeptStatus.
    """
    ...
    edit_owner = models.CharField(
        "Владелец редактирования",
        max_length=10,
        choices=EDIT_OWNER_CHOICES,
        default=EDIT_CROSS,
    )
...
class CrossScheduleDeptStatus(models.Model):
    ...
    в рамках текущей итерации сквозного графика.

    Переходы статусов:
      pending     → in_progress  (отдел принял задание к исполнению)
      in_progress → returned     (отдел вернул на согласование/доработку)
      returned    → in_progress  (задание возвращено отделу на исправление)

    Используется только когда CrossSchedule.granularity = 'per_dept'.
    """
