...
class GGTemplateStage(models.Model):
    ...
    template = models.ForeignKey(
        GGTemplate, on_delete=models.CASCADE,
        related_name='stages', verbose_name='Шаблон',
    )
...
class GeneralSchedule(models.Model):
    ...
    project = models.OneToOneField(
        ...
        related_name='general_schedule', verbose_name='Проект',
    )
    created_by = models.ForeignKey(
        'employees.Employee', on_delete=models.SET_NULL,
        null=True, blank=True, verbose_name='Создал',
...
class GGStage(models.Model):
    ...
    schedule = models.ForeignKey(
        GeneralSchedule, on_delete=models.CASCADE,
        related_name='stages', verbose_name='ГГ',
    )
    ...
    labor = models.DecimalField(
        'Трудозатраты (ч/ч)', max_digits=12, decimal_places=2,
...
class GGMilestone(models.Model):
    ...
    schedule = models.ForeignKey(
        GeneralSchedule, on_delete=models.CASCADE,
        related_name='milestones', verbose_name='ГГ',
    )
    name = models.CharField('Название', max_length=300)
    date = models.DateField('Дата')
    stage = models.ForeignKey(
        GGStage, on_delete=models.SET_NULL,
...
class GGStageDependency(models.Model):
    ...
    predecessor = models.ForeignKey(
        GGStage, on_delete=models.CASCADE,
        related_name='successor_deps', verbose_name='Предшественник',
    )
    successor = models.ForeignKey(
        GGStage, on_delete=models.CASCADE,
        related_name='predecessor_deps', verbose_name='Последователь',
    )
    dep_type = models.CharField(
        'Тип связи', max_length=2, choices=DEP_TYPE_CHOICES, default=DEP_FS,
    )
