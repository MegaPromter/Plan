...
from apps.works.models import Directory, Holiday, WorkCalendar
    ...
    def handle(self, *args, **options):
        ...
        for val in TASK_TYPES:
            ...
            if is_new:
                created_tt += 1
        self.stdout.write(
            f"TaskTypes: {created_tt} создано, {len(TASK_TYPES) - created_tt} уже были"
        )
        ...
        for year, month, hours in CALENDAR_2026:
            ...
            if is_new:
                created_cal += 1
        self.stdout.write(
            f"WorkCalendar: {created_cal} создано, {len(CALENDAR_2026) - created_cal} уже были"
        )
        ...
        for d, name in HOLIDAYS_2026:
            ...
            if is_new:
                created_hol += 1
        self.stdout.write(
            f"Holiday: {created_hol} создано, {len(HOLIDAYS_2026) - created_hol} уже были"
        )
