...
class AbsenceOverlapsView(LoginRequiredJsonMixin, View):
    ...
    def post(self, request):
        ...
        if not isinstance(employee_ids, list) or len(employee_ids) < 2:
            return JsonResponse({'error': 'Нужно выбрать минимум 2 сотрудников'}, status=400)
        ...
        if not include_vacations and not include_trips:
            return JsonResponse({'error': 'Нужно включить хотя бы один тип отсутствия'}, status=400)
        ...
        if include_vacations:
            ...
            for v in qs:
                intervals.append({
                    ...
                    'dept': v.employee.department.code if v.employee.department else '',
                    'start': v.date_start,
                    'end': v.date_end,
                    'type': 'vacation',
        ...
        if include_trips:
            ...
            for t in qs:
                intervals.append({
                    ...
                    'dept': t.employee.department.code if t.employee.department else '',
                    'start': t.date_start,
                    'end': t.date_end,
                    'type': 'trip',
                    'detail': t.location,
        ...
        timeline = sorted(emp_map.values(), key=lambda x: (x['dept'], x['employee_name']))
        ...
        summary = {
            ...
            'max_overlap_days': max((ov['duration_days'] for ov in overlaps), default=0),
        }
...
def _merge_overlaps(overlaps):
    ...
    for ov in overlaps:
        ...
        existing_ids = {(e['employee_id'], e['period_start'], e['period_end']) for e in merged[key]['employees']}
        for emp in ov['employees']:
