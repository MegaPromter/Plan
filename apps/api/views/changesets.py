...
def _snapshot_work(work):
    ...
    return {
        ...
        "row_code": (work.pp_stage.row_code if work.pp_stage_id else work.row_code)
        or "",
        "work_order": (
            work.pp_stage.work_order if work.pp_stage_id else work.work_order
        )
        or "",
...
class ChangesetItemCreateView(APIView):
    ...
    def post(self, request, pk):
        ...
        # Определяем порядок
        max_order = (
            cs.items.order_by("-order").values_list("order", flat=True).first() or 0
        )
