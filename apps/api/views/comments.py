...
def _serialize_comment(c):
    ...
    if c.author:
        ...
        if emp:
            author_name = emp.short_name or emp.full_name or c.author.get_full_name() or c.author.username
        else:
...
class CommentListView(LoginRequiredJsonMixin, View):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse({'error': f'Внутренняя ошибка сервера: {e}'}, status=500)
