    ...
    def get(self, request):
        try:
            ...
            qs = (
                ...
                )  # Сортировка: по краткому, затем полному имени
            )
...
class ProjectMetricsView(APIView):
    ...
    def get(self, request, project_id):
        ...
        works = (
            Work.objects.filter(Q(project=proj) | Q(pp_project_id__in=pp_ids))
            .select_related("executor", "pp_project")
            .prefetch_related("task_executors")
            .annotate(
