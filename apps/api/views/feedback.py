...
def _serialize(fb, current_user=None):
    return {
        'id': fb.pk,
        'user_id': fb.user_id,
        'user_name': (getattr(fb.user, 'employee', None) and fb.user.employee.full_name) or fb.user.get_full_name() or fb.user.username,
        ...
        'screenshots': [{'id': att.pk, 'url': att.image.url} for att in fb.attachments.all()],
