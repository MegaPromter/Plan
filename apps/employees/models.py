...
class Employee(models.Model):
    ...
    position = models.CharField(
        "Должность", max_length=50, choices=POSITION_CHOICES, blank=True
    )
    ...
    def is_writer(self) -> bool:
        ...
        return self.role in (
            self.ROLE_ADMIN,
            self.ROLE_NTC_HEAD,
            self.ROLE_NTC_DEPUTY,
            self.ROLE_DEPT_HEAD,
            self.ROLE_DEPT_DEPUTY,
            self.ROLE_SECTOR_HEAD,
            self.ROLE_CHIEF_DESIGNER,
            self.ROLE_DEPUTY_GD_ECON,
        )
...
class Vacation(models.Model):
    ...
    vac_type = models.CharField(
        "Тип", max_length=10, choices=TYPE_CHOICES, default=TYPE_ANNUAL
    )
    ...
    def __str__(self):
        ...
        return f"{self.employee.short_name}: " f"{self.date_start} – {self.date_end}"
...
class KPI(models.Model):
    ...
    plan_hours = models.DecimalField(
        "Плановые часы", max_digits=7, decimal_places=2, default=0
    )
    # Фактически отработанные/выполненные часы
    fact_hours = models.DecimalField(
        "Фактические часы", max_digits=7, decimal_places=2, default=0
    )
    # Выполнение плана, %  (0–200+)
    # Вычисляется автоматически в методе save()
    completion_pct = models.DecimalField(
        "Выполнение плана, %", max_digits=6, decimal_places=2, null=True, blank=True
    )
    ...
    score = models.DecimalField(
        "Итоговый балл KPI", max_digits=5, decimal_places=2, null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.plan_hours and self.plan_hours > 0:
            ...
            self.completion_pct = (self.fact_hours / self.plan_hours * 100).quantize(
...
class EmployeeDocument(models.Model):
    ...
    doc_type = models.CharField(
        "Тип документа", max_length=20, choices=DOC_TYPE_CHOICES, default=DOC_OTHER
    )
...
class RoleDelegation(models.Model):
    ...
    def __str__(self):
        ...
        return (
            f"{self.delegator.short_name} → {self.delegate.short_name}"
