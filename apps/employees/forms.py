...
class EmployeeForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'user', 'role',                                    # учётная запись и роль
            'last_name', 'first_name', 'patronymic',           # ФИО
            'position', 'ntc_center', 'department', 'sector',  # должность и подразделение
            'phone', 'email_corp',                             # контактные данные
            'hire_date', 'dismissal_date',                     # даты трудовых отношений
            'monthly_hours_norm', 'personal_coeff',            # параметры нормирования
            'must_change_password', 'is_active',               # служебные флаги
        ]
...
class KPIForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'year', 'month',                         # период (год и месяц)
            'plan_hours', 'fact_hours',               # трудоёмкость план/факт
            'norm_control_remarks', 'docs_issued',    # показатели качества
            'score', 'notes',                         # итоговый балл и примечания
        ]
