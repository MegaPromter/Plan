...
_STUB_TITLES = {
    ...
    "projects-deadlines": (
        "Сроки и вехи",
        "Контрольные точки и плановые сроки по проектам",
    ),
    "pp-import": (
        "Импорт / экспорт ПП",
        "Загрузка и выгрузка данных производственного плана",
    ),
    "analytics-workload": (
        "Доска руководителя",
        "Аналитика: загрузка, дедлайны, выполнение плана",
    ),
    "analytics-employee": (
        "Доска сотрудника",
        "Персональные задачи и загрузка по часам",
    ),
    "analytics-pp": (
        "Отчёты ПП",
        "Метрики производственного плана: трудоёмкость, листы, типы работ",
    ),
}
...
class StubView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        title, desc = _STUB_TITLES.get(
            slug, ("Раздел в разработке", "Функциональность находится в разработке")
        )
