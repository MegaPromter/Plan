...
env = environ.Env(
    DEBUG=(
        bool,
        False,
    ),  # если DEBUG не задан в .env — используем False (продакшн-безопасно)
)
...
SECURE_CONTENT_TYPE_NOSNIFF = (
    True  # запрет MIME-sniffing: браузер не угадывает тип контента
)
SECURE_BROWSER_XSS_FILTER = (
    True  # включить фильтр XSS в старых браузерах (заголовок X-XSS-Protection)
)
...
CSRF_COOKIE_SAMESITE = (
    "Lax"  # SameSite=Lax: cookie отправляется при навигации с внешнего сайта
)
