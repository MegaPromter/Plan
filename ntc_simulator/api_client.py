...
class ApiClient:
    ...
    def _request(self, method, path, json_data=None, params=None, max_retries=5):
        ...
        for attempt in range(max_retries + 1):
            ...
            if resp.status_code == 429 and attempt < max_retries:
                wait = 3 * (attempt + 1)
                logger.debug(
                    "429 on %s %s, retry in %ds (attempt %d)...",
                    method,
                    path,
                    wait,
                    attempt + 1,
            ...
            if resp.status_code >= 400:
                logger.warning(
                    "%s %s -> %s: %s", method, path, resp.status_code, resp.text[:200]
                )
            return resp
        return resp
