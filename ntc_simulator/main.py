...
from scenarios.planning import (
    create_pp_plans,
    create_pp_rows,
    create_projects,
    sync_pp_to_sp,
)
from setup import CALENDAR_2026, setup_calendar, setup_org_structure
...
def main():
    ...
    logger.info(
        "Setup done: %d employees, %d departments",
        counters["employees"],
    ...
    for month in range(1, 13):
        ...
        logger.info(
            "Month %02d done in %.1fs: %d tasks, %d reports, %d overdue",
            month,
            month_elapsed,
            total_created,
    ...
    logger.info(
        "Total tasks created over year: %d (PP init: %d, PP monthly: %d, SP: %d)",
        total_tasks,
        counters["pp_rows"],
        counters.get("pp_tasks_created_monthly", 0),
        counters.get("sp_tasks_created_monthly", 0),
    ...
    concurrency_results = test_concurrency(base_url, employees, metrics, config)
