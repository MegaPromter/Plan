...
def simulate_month(client, rng, month, year, org_data, pp_plans, config, metrics):
    ...
    stats["tasks_updated"] = _update_tasks(client, rng, month, year, month_cfg)
    ...
    # 6. Process notices
    stats["notices_processed"] = _process_notices(client, rng, month, year, month_cfg)
    ...
    logger.info(
        "Итог %s: PP+%d, SP+%d (=%d), обнов.%d, срывов=%d, отчётов+%d, отпусков+%d",
        mname,
        stats["pp_tasks_created"],
        stats["sp_tasks_created"],
        total_created,
        stats["tasks_updated"],
        stats["deadlines_missed"],
        stats["reports_created"],
        stats["vacations_created"],
    )
    return stats
...
def _create_dept_tasks(
    ...
    logger.debug(
        "Dept %s %02d/%d: PP=%d SP=%d (target=%d)",
        dept_code,
        month,
        year,
        result["pp_created"],
...
def _create_pp_batch(
    client, rng, month, year, dept_code, sector_codes, emp_by_sector, pp, count, cfg
):
...
def _simulate_deadline_misses(client, rng, month, year, cfg):
    ...
    logger.info(
        "Deadline misses: %d/%d (ratio %.1f%%)", missed, len(eligible), miss_ratio * 100
    )
    return missed
...
def _process_notices(client, rng, month, year, cfg):
    ...
    open_notices = [
        n
        for n in notices
        if not n.get("closure_notice_number") and n.get("ii_pi") == "ИИ"
    ]
        ...
        for _ in range(n_changes):
            ...
            if action == "add":
                ...
                if rows:
                    row = rng.choice(rows)
                    new_end = date(
                        year, min(month + rng.randint(1, 3), 12), rng.randint(1, 28)
                    )
                    resp = client.put(
