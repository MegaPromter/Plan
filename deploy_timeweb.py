...
def main():
    ...
    ssh_exec(ssh, 'apt-get install -y -qq python3-pip python3-venv python3-dev '
    ...
    out, _, _ = ssh_exec(ssh, f"su - postgres -c \"psql -tc \\\"SELECT 1 FROM pg_database WHERE datname='{DB_NAME}'\\\"\"")
    if '1' in out:
        print(f'  База {DB_NAME} уже существует — пропускаем создание')
    else:
        print(f'  База {DB_NAME} не найдена — создаём')
        ssh_exec(ssh, f"su - postgres -c \"psql -c \\\"CREATE USER {DB_USER} WITH PASSWORD '{DB_PASS}';\\\"\"", check=False)
        ssh_exec(ssh, f"su - postgres -c \"psql -c \\\"CREATE DATABASE {DB_NAME} OWNER {DB_USER};\\\"\"", check=False)
        ssh_exec(ssh, f"su - postgres -c \"psql -c \\\"GRANT ALL PRIVILEGES ON DATABASE {DB_NAME} TO {DB_USER};\\\"\"", check=False)

    # 3. Clone/update repo
    print('\n' + '=' * 60)
    print('STEP 3: Clone/update repository')
    print('=' * 60)
    out, _, code = ssh_exec(ssh, f'test -d {APP_DIR}/.git && echo exists || echo missing')
    if 'exists' in out:
        ssh_exec(ssh, f'cd {APP_DIR} && git fetch origin && git reset --hard origin/main')
    else:
...
"""
    ...
    out, _, _ = ssh_exec(ssh, f'test -f {APP_DIR}/.env && echo "exists" || echo "missing"')
    if 'exists' in out:
    ...
    ssh_exec(ssh, f'cd {APP_DIR} && {VENV}/bin/python manage.py collectstatic --noinput')

    # Load data dump if exists
    ssh_exec(ssh, f'cd {APP_DIR} && test -f data_dump.json && {VENV}/bin/python manage.py loaddata data_dump.json || echo "No dump to load"', check=False)
...
"""
    ssh_exec(ssh, f"cat > /etc/systemd/system/planapp.service << 'SVCEOF'\n{gunicorn_service}SVCEOF")
...
"""
    ssh_exec(ssh, f"cat > /etc/nginx/sites-available/planapp << 'NGXEOF'\n{nginx_conf}NGXEOF")
    ssh_exec(ssh, 'ln -sf /etc/nginx/sites-available/planapp /etc/nginx/sites-enabled/planapp')
    ...
    print('=' * 60)
    ssh_exec(ssh, f'certbot --nginx -d {DOMAIN} --non-interactive --agree-tos -m admin@{DOMAIN} --redirect', check=False)
